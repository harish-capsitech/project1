  ...
  return (
    <main className="bg-[#161a22] w-full min-h-screen ml-54 p-10 flex flex-col gap-10">
      ...
      <div className="flex gap-2 w-full">
        <div className="border border-[#262b3a] bg-[#161a22] w-full rounded-xl h-100 overflow-y-auto p-5">
          <h1 className="text-white text-xl font-semibold">Popular Services</h1>
          <div className="flex flex-col gap-3 mt-5">
            {Services.map((data, index)=>(
              <div className="rounded-xl bg-[#1e2036] flex justify-between px-5 py-2 items-center" key={index}>
                <div className="">
                  <h2 className="text-lg font-light text-white">{data.serName}</h2>
                  <p className="text-gray-400">{data.time}</p>
                </div>
                <div className="">
                  <p className="text-lg font-light text-white">{data.price}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="border border-[#262b3a] bg-[#161a22] w-full rounded-xl h-100 overflow-y-auto p-5">
          ...
          <div className="flex flex-col gap-3 mt-5">
            {staff.map((data, index)=>(
              <div className="  rounded-xl bg-[#1e2036] flex justify-between px-5 py-2 items-center " key={index}>
                <div className="">
                  <h2 className="text-lg font-light text-white">{data.empName}</h2>
                  <p className="text-gray-400">{data.empEmail}</p>
                </div>
                <div className="flex flex-col items-end">
                  <p className="text-lg font-light text-white">{data.bookings}</p>
                  <p className="font-extralight text-xs text-gray-400">Bookings</p>
                </div>
              </div>
            ))}
          </div>
